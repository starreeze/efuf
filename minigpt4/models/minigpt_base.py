...
class MiniGPTBase(BaseModel):
    ...
    def prompt_wrap(self, img_embeds, atts_img, prompts, lengths=None):
        if prompts is None or len(prompts) == 0:
            ...
            prompt_tokens = self.llama_tokenizer(
                prompts, return_tensors="pt", padding="longest", add_special_tokens=False
            ).to(self.device)
            ...
            for idx, (each_img_embed, each_prompt) in enumerate(zip(img_embeds, prompts)):
                ...
                for idx, seg in enumerate(p_segs[:-1]):
                    p_tokens = self.llama_tokenizer(seg, return_tensors="pt", add_special_tokens=False).to(
                        img_embeds.device
                    )
                    p_embed = self.embed_tokens(p_tokens.input_ids)
                    interleave_emb.append(
                        torch.cat([p_embed, each_img_embed[None][:, idx * pn : (idx + 1) * pn]], dim=1)
                    )
                wrapped_emb = torch.cat(interleave_emb, dim=1)
                p_tokens = self.llama_tokenizer(p_segs[-1], return_tensors="pt", add_special_tokens=False).to(
                    img_embeds.device
                )
                p_embed = self.embed_tokens(p_tokens.input_ids)
    ...
    def tokenize_conversation(self, conv_q, conv_a):
        ...
        for batch_idx in range(batch_size):
            ...
            questions = [
                self.llama_tokenizer(
                    self.llama_tokenizer.bos_token + q, return_tensors="pt", add_special_tokens=False
                ).to(self.device)
                for q in questions[1:]
            ]  # the first question is handled in the prompt wrap function, skip it
            answers = [
                self.llama_tokenizer(a + self.end_sym, return_tensors="pt", add_special_tokens=False).to(self.device)
                for a in answers
            ]
            cur_id = []
            cur_target = []
        ...
        to_regress_token_ids = (
            torch.ones([batch_size, max_len], dtype=cur_id.dtype, device=self.device)
    ...
    def preparing_embedding(self, samples, add_end_sym: list[bool]):
        ...
        if "conv_q" in samples:
            ...
            self.llama_tokenizer.padding_side = "right"
            text = [t + self.end_sym if aes else t for t, aes in zip(samples["answer"], add_end_sym)]
    ...
    def forward(self, samples, add_end_sym: list[bool], reduction="mean"):
        ...
        cond_embeds, cond_atts, regress_embeds, regress_atts, part_targets = self.preparing_embedding(
            samples, add_end_sym
        )
        ...
        targets = (
            torch.ones([inputs_embeds.shape[0], inputs_embeds.shape[1]], dtype=torch.long).to(self.device).fill_(-100)
        )
