...
import os, sys
sys.path.append('/workspace/hal/code/Owl/')
sys.path.append('/workspace/hal/Owl/')
# sys.path.append('/data/ant/code/Owl/mplug_owl/')
# sys.path.append('/data/ant/code/Owl/pipeline/')
# sys.path.append('/data/ant/code/')
...
def do_generate(prompts, image_list, model, tokenizer, processor, use_bf16=False, **generate_kwargs):
    ...
    print(f"inputs={inputs}, inputs.shape={inputs['pixel_values'].shape}")
...
AI: ''']
    image_list = ['/workspace/hal/Owl/examples/monday.jpg']
    # base_model = 'MAGAer13/mplug-owl-llama-7b'  
    base_model= '/workspace/hal/checkpoints/mplug_OWL_llama_7b' 
